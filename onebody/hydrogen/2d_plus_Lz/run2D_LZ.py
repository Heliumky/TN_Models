...
if __name__ == '__main__':
    ...
    #H, L, R  = npmps.sum_2MPO (H, L, R, Lz, 1e-5*LLz, RLz)
    ...
    # Stack effect:
    H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-15)
    #H, L, R  = npmps.sum_2MPO (H, L, R, cos, -1e-4*Lcos, Rcos)
    H, L, R  = npmps.sum_2MPO (H, L, R, xdr, -1e-4*Lxdr, Rxdr)
    H, L, R  = npmps.change_dtype (H, L, R, complex)
    ...
    np.savetxt(f'2d_terr0N={N}.dat',np.column_stack((terrs0,ens0)))


    #maxdims = maxdims + [64] * 20
    maxdims = [2]*20 + [4]*100 + [8]*200 
    ...
    psi1,ens1,terrs1 = dmrg.dmrg (psi1, H, L, R, maxdims, cutoff, ortho_mpss=[psi0], weights=[6])
    np.savetxt(f'2d_terr1={N}.dat',np.column_stack((terrs1,ens1)))
