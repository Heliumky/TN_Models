...
input_mps_path = "2d_ED_results_N=7.pkl"
...
for i, (Z, title) in enumerate(zip(wavefunctions, titles)):
    ...
    # y=0 cut (horizontal)
    plt.subplot(1, 2, 1)
    plt.plot(xs, Z.real[iy0, :] / dx, label=f'Re($\psi$)')
    plt.plot(xs, (np.abs(Z[iy0, :] / dx))**2, label=f'$|\psi|^2$', linestyle='--')
    ...
    # x=0 cut (vertical)
    plt.subplot(1, 2, 2)
    plt.plot(ys, Z.real[:, ix0] / dx, label='Re($\psi$)')
    plt.plot(ys, (np.abs(Z[:, ix0] / dx))**2, label='$|\psi|^2$', linestyle='--')
    ...
    plt.tight_layout()
    plt.savefig(f"2d_ED_cut_{title}_N{N}.pdf", format='pdf')
    plt.close()
