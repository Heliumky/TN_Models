...
if __name__ == '__main__':
    ...
    # Stack effect:
    #H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-12)
    #H, L, R  = npmps.sum_2MPO (H, L, R, Ex, -1e-5*LEx, REx)
    #H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-12)
    #H, L, R  = npmps.change_dtype (H, L, R, complex)

    # Stack effect:
    #H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-12)
    #H, L, R  = npmps.sum_2MPO (H, L, R, cos, -1e-5*Lcos, Rcos)
    H, L, R  = npmps.sum_2MPO (H, L, R, xdr, -1e-5*Lxdr, Rxdr)
    H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-12)
    H, L, R  = npmps.change_dtype (H, L, R, complex)

    # choose special Lz2 quantum number:
    #H, L, R  = npmps.sum_2MPO (H, L, R, Lz2, 1e-2*LLz2, RLz2)
    #H, L, R = npmps.compress_MPO (H, L, R, cutoff=1e-12)
    #H, L, R  = npmps.change_dtype (H, L, R, complex)
    ...
    maxdims = [2]*20 + [4]*400 + [8]*700+ [16] * 50
