...
E0 = np.loadtxt(f"3d_terr0_N={N}.dat", dtype=np.complex128)
E1 = np.loadtxt(f"3d_terr1_N={N}.dat", dtype=np.complex128)
...
# Main plot curves
ax.plot(range(len(E0[1])), np.real(E0[2]), color='red',ls = "solid", label=r"GS decay")
ax.axhline(y=-1/2, color='red', linestyle='--', linewidth=1.5, label=r"Exact GSE=-2")
...
ax_inset.set_ylim(-1/6, 0)
#ax_inset.set_xlim(0,8*1e3)
# Inset plot curves
ax_inset.plot(range(len(E1[1])), np.real(E1[2]), color='black',ls = "solid", label=r"FS decay")
ax_inset.axhline(y=-1/8, color='black', linestyle='--', linewidth=1.5, label=r"$Exact FSE-2/9$")
# Inset labels
