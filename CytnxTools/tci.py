...
def interact_1D_SHO (inds, fac, rescale, shift, cutoff, R):
    N = len(inds)
    xx1 = inds[::2]
    xx2 = inds[1::2]
    x1 = cc_inds_to_x(xx1, rescale, shift)
    x2 = cc_inds_to_x(xx2, rescale, shift)
    x = cc_inds_to_x (inds, rescale, shift)
    dx = x2-x1
    r = 0.5*dx**2
    return r

def interact_1D_Hmol (inds, fac, rescale, shift, cutoff, R):
    N = len(inds)
    xx1 = inds[::2]
    xx2 = inds[1::2]
    x1 = cc_inds_to_x(xx1, rescale, shift)
    ...
    x = cc_inds_to_x (inds, rescale, shift)
    r = np.sqrt(dx**2)
    r_safe = np.where(r < cutoff, cutoff, r)
    return fac / r_safe
...
def interact_2D_Hmol(inds, fac, rescale, shift, cutoff, R):
    xx1 = inds[::2]
    xx2 = inds[1::2]
    N = len(xx1)//2
    ...
    r_safe = np.where(r < cutoff, np.sqrt(0.5)*cutoff, r)
    return fac / r_safe

def interact_2D_SHO(inds, fac, rescale, shift, cutoff, R):
    xx1 = inds[::2]
    xx2 = inds[1::2]
    N = len(xx1)//2
...
def one_over_r_3D (inds, fac, rescale, shift, cutoff, R):
    N = len(inds)
    Ni = N//3
    inds_x = inds[:Ni]
    inds_y = inds[Ni:2*Ni]
...
def interact_3D_Hmol(inds, fac, rescale, shift, cutoff, R):
    xx1 = inds[::2]
    xx2 = inds[1::2]
    N = len(xx1)//3

    x1 = cc_inds_to_x(xx1[:N], rescale, shift)
    y1 = cc_inds_to_x(xx1[N:2*N], rescale, shift)
    z1 = cc_inds_to_x(xx1[2*N:], rescale, shift)
    x2 = cc_inds_to_x(xx2[:N], rescale, shift)
    y2 = cc_inds_to_x(xx2[N:2*N], rescale, shift)
    z2 = cc_inds_to_x(xx2[2*N:], rescale, shift)
    ...
    r_safe = np.where(r < cutoff, np.sqrt(0.75)*cutoff, r)
    return fac / r_safe

def interact_3D_SHO(inds, fac, rescale, shift, cutoff, R):
    xx1 = inds[::2]
    xx2 = inds[1::2]
    N = len(xx1)//3
...
if __name__ == '__main__':
    ...
    if '--1D_one_over_r' in sys.argv:
        ...
        #plotF(mps, target_func, rescale, shift)
    elif '--2D_interact_SHO' in sys.argv:
        def target_func (inds):
            return interact_2D_SHO (inds, factor, rescale, shift, cutoff, R)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor, R)
        #plotF(mps, target_func, rescale, shift)    
    elif '--2D_interact_Hmol' in sys.argv:
        def target_func (inds):
            return interact_2D_Hmol (inds, factor, rescale, shift, cutoff, R)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor, R)
        #plotF(mps, target_func, rescale, shift)    
        ...
        def target_func (inds):
            return one_over_r_3D (inds, factor, rescale, shift, cutoff, R)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor, R)
    elif '--3D_interact_Hmol' in sys.argv:
        def target_func (inds):
            return interact_3D_Hmol (inds, factor, rescale, shift, cutoff, R)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor, R)
        #plotF(mps, target_func, rescale, shift)  
    elif '--3D_interact_SHO' in sys.argv:
        def target_func (inds):
            return interact_3D_SHO (inds, factor, rescale, shift, cutoff, R)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor, R)
        #plotF(mps, target_func, rescale, shift)  
    else:
        raise Exception
